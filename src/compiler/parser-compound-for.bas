...
private sub hScalarNext _
    ...
    '' is comparison op known?
    if( stk->for.explicit_relop ) then
    	'' counter <= or >= end cond?
		hFlushBOP( stk->for.relop, _
			   	   @stk->for.cnt, _
			   	   @stk->for.end, _
			   	   stk->for.inilabel )
...
private sub hForTo _
	...
	stk->for.explicit_relop = FALSE
	if( (flags and FOR_ISUDT) = 0 ) then
		'' '>' | '<' | '>=' | '<='
		tk = lexGetToken( )
		select case as const tk
		case FB_TK_GT, FB_TK_LT, FB_TK_LE, FB_TK_GE
			stk->for.explicit_relop = TRUE
			stk->for.relop = hFBrelop2IRrelop( tk )
			lexSkipToken( )
		end select
	end if
...
private sub hForStep _
	...
	if( (flags and FOR_HASCTOR) = 0 ) then
		...
		if( stk->for.explicit_step ) then
			...
			if( stk->for.explicit_relop ) then
				select case stk->for.relop
				case AST_OP_GT, AST_OP_GE
					stepval = -1
				end select
			end if
			expr = astNewCONSTi( stepval )
		end if
    ...
    elseif( iscomplex and ((flags and FOR_ISUDT) = 0) and stk->for.explicit_relop = FALSE ) then
