...
private sub _emitADDF_SSE _
	...
	if( svreg->typ = IR_VREGTYPE_REG ) or ( symbGetDataClass( svreg->dtype ) = FB_DATACLASS_FPOINT ) then
		if( ddsize > 4 ) then
			if( sdsize > 4 ) then
				'' add them as double-precision
				outp "addsd " + dst + ", " + src
			else
				'' convert src to double, then add
				outp "cvtss2sd xmm7, " + src
				outp "addsd " + dst + ", xmm7"
			endif
		else
			if( sdsize > 4 ) then
				'' convert src to single, then add
				outp "cvtsd2ss xmm7" + COMMA + src
	...
	if( svreg->typ = IR_VREGTYPE_REG ) or ( symbGetDataClass( svreg->dtype ) = FB_DATACLASS_FPOINT ) then
		if( ddsize > 4 ) then
			if( sdsize > 4 ) then
				...
				'' convert src to double, then subtract
				outp "cvtss2sd xmm7, " + src
				outp "subsd " + dst + ", xmm7"
			endif
		else
			if( sdsize > 4 ) then
				'' convert src to single, then subtract
				outp "cvtsd2ss xmm7, " + src
				outp "subss " + dst + ", xmm7"
			else
	...
	if( symbGetDataClass( svreg->dtype ) = FB_DATACLASS_FPOINT ) then
		if( ddsize > 4 ) then
			if( sdsize > 4 ) then
				...
				'' convert src to double, then multiply
				outp "cvtss2sd xmm7, " + src
				outp "mulsd " + dst + ", xmm7"
			endif
		else
			if( sdsize > 4 ) then
				'' convert src to single, then multiply
				outp "cvtsd2ss xmm7, " + src
				outp "mulss " + dst + ", xmm7"
			else
	...
	if( symbGetDataClass( svreg->dtype ) = FB_DATACLASS_FPOINT ) then
		...
		if( ddsize > 4 ) then
			if( sdsize > 4 ) then
				...
				'' convert src to double, then divide
				outp "cvtss2sd xmm7, " + src
				outp "divsd " + dst + ", xmm7"
			endif
		else
			if( sdsize > 4 ) then
				'' convert src to single, then divide
				outp "cvtsd2ss xmm7, " + src
				outp "divss " + dst + ", xmm7"
			else
				'' divide them as single-precision
				outp "divss " + dst + ", " + src
				'outp "rcpss xmm7" + COMMA + src
